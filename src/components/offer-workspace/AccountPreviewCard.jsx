...
export default function AccountPreviewCard({
  ...
  const empName = watchedValues?.employeeName || employeeName || '';
  const empId = watchedValues?.employeeId || employeeId || '';
  ...
  return (
    <div className="bg-white rounded-2xl border border-[var(--color-border)] p-4 sm:p-6 shadow-xs space-y-4 min-w-0 max-w-full overflow-hidden">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-[var(--color-border)] pb-3">
        ...
        <span className="text-[10px] text-gray-400 font-mono font-medium self-start sm:self-auto shrink-0">System Provisioned</span>
      </div>

      <div className="bg-gray-50/90 rounded-xl p-4 border border-gray-200 space-y-3 text-xs">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <span className="text-xs uppercase font-bold text-gray-500 shrink-0">Assigned Username:</span>
          <div className="flex items-center justify-between sm:justify-start gap-2 font-mono text-[var(--color-primary)] font-bold text-xs sm:text-sm bg-white px-3 py-1.5 rounded-lg border border-blue-200 min-w-0">
            <span className="truncate break-all">{username}</span>
            <button
              type="button"
              onClick={handleCopy}
              className="p-1 text-gray-400 hover:text-[var(--color-primary)] rounded shrink-0 cursor-pointer"
              title="Copy username"
            >
